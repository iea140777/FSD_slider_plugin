...
let sliderContainer = document.querySelector('.slider__container'),
    ...
    currentPosition = [],
    range = true,
    rangeStart = 1, //порядковый номер бегунка с которого начинается диапазон
    rangeEnd = 2, //порядковый номер бегунка на котором заканчивается диапазон
    handlersAmount = 3;
    ...
    handlers.forEach(function(item){
        item.addEventListener('mousedown', function(event) {
            ...
            function onMouseMove(event) {
                ...
                if (range){
                    getCurrentRange(rangeStart, rangeEnd);
                    getCurrentRange(rangeStart, rangeEnd);
                };
